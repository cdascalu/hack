...
final class BarcodeQR {
	 ...
	 * @return bool
	 */
	public function draw($size = 150, $filename = 'default.jpg') {
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, self::API_CHART_URL);
		curl_setopt($ch, CURLOPT_POST, true);
		curl_setopt($ch, CURLOPT_POSTFIELDS, "chs={$size}x{$size}&cht=qr&chl=" . urlencode($this->_data)."&chld=L|3");
		...
		switch($ext){
			case "jpg":
					$banner=imagecreatefromjpeg($filename);
			break;
			...
			case "png":
					$banner=imagecreatefrompng($filename);
			break;
		}
		$qr = imagecreatefrompng('qr.png');
		
		imagecopymerge ( $banner ,$qr , 0 , 0 , 10 , 10 , 90 , 90 , 100 );
